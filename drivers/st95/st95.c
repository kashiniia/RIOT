...
void st95_spi_reset(const st95_t * dev)
{
    ...
    ztimer_sleep(ZTIMER_MSEC, ST95_HFO_SETUP_TIME_MS);  
}
...
static void _st95_wait_ready_data(void)
{
    uint32_t time_begin = ztimer_now(ZTIMER_MSEC);
    uint32_t time_end = 0;
	...
	while((st95_state.data_rx == false) && (st95_state.timeout == false)) {
		time_end = ztimer_now(ZTIMER_MSEC);
		time_delta = time_end - time_begin;
        //printf("time delta %li\n", time_delta);
...
{      
    ...
    do {
        ...
        ztimer_sleep(ZTIMER_MSEC, ST95_DELAY_POWER_ON_MS);
            
            /* ST95 Power On */
        gpio_clear(dev->params.vcc);
            /* Ramp-up time from 0V to Vps */
        ztimer_sleep(ZTIMER_MSEC, ST95_RAMP_UP_TIME_MS);
